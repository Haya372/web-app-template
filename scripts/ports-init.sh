...
if [ -f "$ENV_FILE" ]; then
  grep -v '^WORKTREE_OFFSET=\|^APP_PORT=\|^APP_GRPC_PORT=\|^VITE_PORT=\|^NEXT_PORT=\|^DB_PORT=\|^CORS_ALLOW_ORIGINS=' "$ENV_FILE" > "$ENV_TMP" || true
else
  > "$ENV_TMP"
fi

{
  ...
  echo "NEXT_PORT=$((3001 + OFFSET))"
  echo "DB_PORT=$((55432 + OFFSET))"
  echo "CORS_ALLOW_ORIGINS=http://localhost:$((3000 + OFFSET))"
} >> "$ENV_TMP"

mv "$ENV_TMP" "$ENV_FILE"
echo "Done: APP_PORT=$((8080+OFFSET)) APP_GRPC_PORT=$((8081+OFFSET)) VITE_PORT=$((3000+OFFSET)) NEXT_PORT=$((3001+OFFSET)) DB_PORT=$((55432+OFFSET))"
