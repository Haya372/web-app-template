...
- Import files directly by path — both within a feature and across features. `index.ts` barrel re-exports are forbidden within `src/`. Exception: `@repo/ui` intentionally exposes its public API through `index.ts` — always import from `@repo/ui` (e.g. `import { Button } from "@repo/ui"`), not from its internal paths.
...
- Use Biome for formatting (tabs, double quotes for JSX strings) and linting; ESLint enforces additional quality rules — always pass `pnpm lint` before pushing. See `eslint.config.mjs` for disabled rules and their rationale.
