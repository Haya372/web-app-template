...
Prompt: コンテキストファイルの「Review prompt」セクションを参照してプロンプトを構築する。
        ...
        - Domain doc updates: ドメインの概念・業務ルール・用語の変更があった場合、`docs/context/domains/` と `docs/context/glossary.md` の更新が含まれているか確認すること
```
...
### Step 6: ドメイン知識スナップショットを更新する

実装でドメインの **概念・業務ルール・用語** が変わった場合、関連する `docs/context/domains/<entity>.md` と `docs/context/glossary.md` を更新する。

**更新が必要なケース:**
- 新しいエンティティ/集約を追加した → `docs/context/domains/<entity>.md` を新規作成（`_template.md` をコピー）
- 既存ドメインのビジネスルールや状態遷移を変更した → 該当 `domains/<entity>.md` を更新
- ドメイン用語を追加・変更した → `glossary.md` を更新

**更新不要なケース（理由を 1 行記録して次へ）:**
- 純粋な内部リファクタリング（ドメインルール不変）
- UseCase 層の追加のみ
- API エンドポイントの追加のみ

書く内容と書かない内容の詳細は `docs/context/README.md` を参照すること。
...
```
Skill("commit", "#<issue-number> checkpoint: <task description>")
```
