...
1. **WORKTREE_OFFSET**: 各 worktree に固有の整数オフセット（0, 100, 200, …）を割り当てる。各サービスのポートは `BASE_PORT + WORKTREE_OFFSET` で決定する。
   ...
   | Go Connect-RPC | 8081 | 8081 | 8181 | 8281 |
   | React フロントエンド | 3000 | 3000 | 3100 | 3200 |
   | Next.js フロントエンド | 3001 | 3001 | 3101 | 3201 |
...
- `.env.local` 内のポート関連キー（`WORKTREE_OFFSET`, `APP_PORT`, `APP_GRPC_PORT`, `VITE_PORT`, `NEXT_PORT`, `DB_PORT`, `CORS_ALLOW_ORIGINS`）は `ports:init` が管理するため手動編集しない。
