...
- `@connectrpc/connect` + `@connectrpc/connect-node` を使用する（BFF は Node.js ランタイムで動作するため）
...
- ブラウザから直接 Connect-RPC を呼び出す要件が発生した場合は別途 `@connectrpc/connect-web` の採用を検討する
...
BFF は Node.js プロセスとして動作するため、`@connectrpc/connect-web` の `fetch` ベース API ではなく `@connectrpc/connect-node` の HTTP/2 直結トランスポートを採用する。サーバ間通信のコネクション再利用とストリーミング対応への拡張性を確保するため、`connect-node` の方が適切である。
...
- **Migration**:
  ...
  - Issue #106 で BFF 側の Connect-RPC 統合を実施し、初期実装として `@connectrpc/connect-node` を採用する
  - Node 側 monorepo では `@bufbuild/buf` を npm 経由で導入し、Go 側 CI（`bufbuild/buf-action` + `mise.toml`）と minor バージョンを揃える運用とする（buf を bump する際は `mise.toml`、`bufbuild/buf-action` の version、`@bufbuild/buf` の package.json 指定を同 PR で更新する）
